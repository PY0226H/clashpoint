...
| `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_judge_mainline.py tests/test_judge_command_routes.py tests/test_runtime_readiness_public_contract.py tests/test_public_verify_projection.py tests/test_trust_challenge_public_contract.py` | `pass` |
| `cd ai_judge_service && ../scripts/py -m ruff check app tests scripts` | `pass` |
| `cd ai_judge_service && ../scripts/py -m pytest -q` | `pass` |
...
5. plan consistency gate：`artifacts/harness/ai-judge-local-reference-plan-consistency.summary.md`
