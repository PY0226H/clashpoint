...
1. 根据最新产品决策，`NPC Coach` 与 `Room QA` 相关的一切开发内容全部暂停。
2. 本方案当前只作为官方 AI 裁判庭 / `Official Verdict Plane` 的设计依据；不得从本方案或关联方案中继续外推 `NPC Coach`、`Room QA`、用户辩论助手或低延迟交互型 Agent 开发任务。
3. 若后续重新启用相关能力，必须先以独立 PRD 和独立模块设计重新冻结范围，再进入开发计划。
