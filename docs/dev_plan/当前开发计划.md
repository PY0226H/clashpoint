...
当前状态：进行中（`ai-judge-cases-hard-cut` 已完成，`ai-judge-judge-core-unification` 已完成，`ai-judge-contract-hardening` phase1+phase2 已完成，`ai-judge-cross-layer-sync` phase1+phase2 已完成，`ai-judge-case-evidence-view` phase1 已完成，`ai-judge-runtime-ops-pack` phase1 已完成，`ai-judge-doc-governance-refresh` 已完成）
...
1. `ai-judge-stage-closure-evidence`
2. `ai-judge-runtime-ops-pack`（phase2：与 stage closure 文档联动自动回填）
3. `ai-judge-real-env-window-closure`（真实环境窗口可用时执行）
...
| M4：收口与证据归档 | 进行中 | 已完成 runtime ops pack phase1 + doc governance refresh；待完成 stage closure evidence 与 phase2 自动回填 |

---

## 9. 下一步（执行入口）

1. 推进 `ai-judge-stage-closure-evidence`，准备本阶段收口草案与 deferred debt 列表。
2. 推进 `ai-judge-runtime-ops-pack`（phase2），将收口草案和 runtime 收口包做自动回填联动。
3. 在真实环境窗口可用时推进 `ai-judge-real-env-window-closure`，补齐 real-env 证据冻结结论。
