...
当前状态：执行中（已完成本轮计划重置与旧计划归档；`ai-judge-p32-app-factory-hotspot-split-v3` 已完成前两批：registry governance 路由下沉 + NPC/Room QA 路由下沉）
...
| `ai-judge-p32-app-factory-hotspot-split-v3` | P0 | 进行中 | 继续完成 `app_factory` 热点拆分（承接 P31 前二十七批） | 前两批已完成（2026-04-22）：1）新增 `app/applications/registry_governance_routes.py`，下沉 `dependency health/governance overview/prompt-tool governance/domain families/gate simulation` 路由编排，`dependency health` 与 `gate simulation` 已知错误映射改走 `RegistryRouteError -> _run_registry_route_guard`；2）新增 `app/applications/assistant_agent_routes.py`，下沉 `NPC Coach / Room QA` 路由编排并新增 `_run_assistant_agent_route_guard` 统一 `invalid_session_id` 错误映射；后续继续下沉 command/read/panel/review/alert 热点 | `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_registry_governance_routes.py tests/test_assistant_agent_routes.py tests/test_app_factory.py -k "policy_registry_dependency_health_route or policy_gate_simulation_route or registry_governance_overview_route or registry_prompt_tool_governance_route or policy_domain_judge_families_route or npc_coach or room_qa"` + `cd ai_judge_service && ../scripts/py -m ruff check app/app_factory.py app/applications/registry_governance_routes.py app/applications/assistant_agent_routes.py tests/test_registry_governance_routes.py tests/test_assistant_agent_routes.py` |
| `ai-judge-p32-judge-command-route-orchestrator-v1` | P0 | 待执行 | 下沉 `cases create + phase/final dispatch + blindization reject` 命令编排 | `app_factory` 不再承载命令链核心业务分支；dispatch/callback/receipt/error 路由语义与现有测试一致 | `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_app_factory.py -k "case_create or phase_dispatch or final_dispatch or failed_callback or blindization"` |
