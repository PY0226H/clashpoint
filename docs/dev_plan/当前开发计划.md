...
当前状态：进行中（M4 `ai-judge-trust-layer-phaseA` 已完成，进入 M5 `ai-judge-cross-layer-sync-v2`）
...
| `ai-judge-cross-layer-sync-v2` | 跨层二次同步 | 进行中 | 下一模块执行中 |
...
1. `ai-judge-cross-layer-sync-v2`
2. `ai-judge-npc-qa-shell-bootstrap`
3. `ai-judge-real-env-window-pass-closure`
