...
当前状态：执行中（P41 首批 runtime readiness contract / chat proxy / Ops Console 已落地；下一步进入 calibration decision log 与 panel shadow candidate contract）
...
| AI 内部 Ops Pack | `GET /internal/judge/ops/read-model/pack` 已汇总 `fairnessCalibrationAdvisor`、`panelRuntimeReadiness`、`release/trust/challenge` 等内部读模型；`/internal/judge/ops/runtime-readiness` 已提供 public-safe readiness 投影 | 首批 runtime readiness contract 已冻结；下一步应把 calibration decision、panel candidate 与 release evidence 做成治理动作闭环 |
| Fairness / Calibration | AI 服务已有 `/internal/judge/fairness/calibration-pack` 与 `/internal/judge/fairness/policy-calibration-advisor`，并进入 `ops_read_model_pack.py` | 目前偏内部 advisory/read path；下一步需要可审计的 calibration decision log，且明确不自动改官方 winner |
| Panel Runtime / Shadow | AI 服务已有 `/internal/judge/panels/runtime/profiles` 与 `/readiness`，包含 shadow agreement、成本、时延、候选模型与 release signal | 已具备生产化前的可观测输入，但还缺“候选模型可否进入 rollout”的稳定合同和 Ops 展示 |
| Registry / Release Gate | `route_group_registry.py` 已提供 policy/prompt/tool registry、publish/activate/rollback/audit/release 入口；`registry_release_gate.py` 已收敛 release readiness evidence | release gate 已有内部依据，P41 需要让 chat Ops 以白名单形式读取关键状态，而不是要求客户端直连 AI 服务 |
| chat_server 门面 | 已有 `/api/debate/sessions/{id}/judge-report/public-verify`、`/challenge`、`/challenge/request`、`/api/debate/ops/judge-challenge-queue` 与 `/api/debate/ops/judge-runtime-readiness` | runtime readiness 已接入主业务 Ops 门面；下一步缺 calibration decision / panel candidate 的治理写入或更细读模 |
| 前端/SDK | `debate-domain` 与 `DebateRoomPage` 已接入 public verify/challenge；`ops-domain` 与 `OpsConsolePage` 已展示 judge challenge queue 与 runtime readiness | 首批 readiness 首屏已完成；下一步补人工 decision log、candidate readiness 与 stage closure evidence 展示 |
| 文件热点 | 当前热点：`ops_read_model_pack.py`、`registry_routes.py`、`final_report.py`、`request_report_query.rs`、`OpsConsolePage.tsx` 仍偏大 | 已通过新增 AI projection 与 chat projection 控制一部分膨胀；后续 P41 仍需继续下沉 frontend resolver 与 chat helper |
...
| `ai-judge-p41-calibration-decision-log-pack` | 建立 calibration recommendation 的人工决策日志 | 待执行 | 记录 accept/reject/defer，不自动应用 policy；为后续 auto-calibration 打地基 |
| `ai-judge-p41-panel-shadow-candidate-contract-pack` | 稳定多模型 shadow 候选合同 | 待执行 | 冻结 candidate model/profile、成本、时延、drift、switch blockers；不启用生产自动切换 |
| `ai-judge-p41-release-readiness-ops-evidence-pack` | 将 release readiness artifact 与 Ops 代理证据归一 | 待执行 | 让 release gate、runtime ops pack、public verify/challenge 与 P41 ops bridge 证据互相引用 |
| `ai-judge-p41-route-hotspot-split-pack` | 控制 P41 新增 read-model / proxy / UI 热点膨胀 | 待执行 | AI projection、chat model/helper、frontend domain resolver 分层下沉 |
...
1. ai-judge-p41-calibration-decision-log-pack
2. ai-judge-p41-panel-shadow-candidate-contract-pack
3. ai-judge-p41-release-readiness-ops-evidence-pack
...
| Batch-E | `calibration-decision-log` | 把 calibration advisor 从“只读建议”推进为可审计人工决策 | accept/reject/defer 有稳定状态、actor、reason、source artifact，不自动应用 policy |
| Batch-F | `panel-shadow-candidate-contract` | 稳定 shadow candidate model/product readiness 合同 | 多模型候选只进入 readiness/read model，不切官方裁决 |
| Batch-G | `release-readiness-ops-evidence` | 归一 P41 代理与 release readiness artifact 证据 | release artifact 能引用 P41 ops bridge 状态，仍区分 local/reference/real-env |
| Batch-H | `route-hotspot-split + regression + stage-closure` | 控制热点并完成本地证据收口 | 本地门禁通过；real-env 仍阻塞时只进入 todo，不宣称 pass |
...
1. calibration recommendation 能被人工记录决策。
2. 决策日志可查询、可回放、可被 release gate 引用。
3. 不存在自动 activate policy/prompt/tool 的路径。
4. local reference only 的建议不能被标记为 production ready。

**建议回归**

1. `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_route_group_fairness.py tests/test_registry_release_gate.py`
2. `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_policy_registry_runtime.py tests/test_registry_governance_routes.py`
...
1. panel runtime readiness 对多模型候选有稳定、可测试的 contract。
2. 输出明确声明 `does not change official winner semantics` 或等效字段。
3. release gate 可以读取 shadow candidate summary。
4. chat/frontend 只能看到 public-safe 摘要。

**建议回归**

1. `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_panel_runtime_profile_contract.py tests/test_route_group_panel_runtime.py tests/test_panel_runtime_routes.py`
2. `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_registry_release_gate.py`
...
1. release readiness evidence 能显示 P41 control plane status。
2. `env_blocked` 仍保持阻塞，不被 P41 local reference 误抬为 pass。
3. stage closure 草案能识别 P41 完成模块与真实环境后置项。
4. 输出 artifact ref / manifest hash / decision / storage mode 可追溯。

**建议回归**

1. `bash scripts/harness/ai_judge_runtime_ops_pack.sh --root /Users/panyihang/Documents/EchoIsle --allow-local-reference`
2. `bash scripts/harness/ai_judge_stage_closure_evidence.sh --root /Users/panyihang/Documents/EchoIsle`
3. `bash scripts/harness/tests/test_ai_judge_runtime_ops_pack.sh`
...
1. P41 新增逻辑不继续显著堆大 `ops_read_model_pack.py`、`request_report_query.rs`、`OpsConsolePage.tsx`。
2. 新增 helper 有独立 targeted tests。
3. route/handler 文件保持薄装配，不做大段 JSON 拼装。
4. 无行为改动的拆分通过 targeted regression。

**建议回归**

1. `cd ai_judge_service && ../scripts/py -m ruff check app tests`
2. `cd ai_judge_service && ../scripts/py -m pytest -q tests/test_ops_read_model_pack.py tests/test_route_group_ops_read_model_pack.py`
3. `cd chat && cargo test -p chat-server ai_judge_runtime_readiness`
...
1. 先执行 `ai-judge-p41-runtime-readiness-contract-freeze-pack`，把 AI 内部 ops read model pack 收敛成 public-safe runtime readiness 合同。
2. 再执行 `ai-judge-p41-chat-runtime-readiness-proxy-pack`，让 Ops 通过 `chat_server` 读取 readiness，而不是直连 AI 服务。
3. 然后执行 `ai-judge-p41-client-runtime-readiness-console-pack`，让 Ops Console 展示 release gate、calibration、panel shadow 与 real-env 阻塞状态。
