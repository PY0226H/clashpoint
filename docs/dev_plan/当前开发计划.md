...
2. 准备真实样本 manifest、provider、callback 与 readiness 输入，执行 `ai-judge-real-env-pass-window-execute-on-env`。
3. 继续维护官方 Judge trust/fairness/review/ops 闭环；`NPC Coach` / `Room QA` 保持暂停。
