...
当前状态：执行中（Batch-A `artifact-store-production-readiness` 已完成，下一步建议执行 public verification boundary）
...
| `ai-judge-p37-public-verification-boundary-pack` | Public Verification 外部化前的 payload、token、URI 与 chat 代理边界 | 未开始 | 不开放客户端直连 AI 服务；先稳定内部可代理合同 |
| `ai-judge-p37-real-env-evidence-readiness-pack` | 真实环境窗口输入、脚本、证据目录与阻塞项解释 | 未开始 | 为真实窗口做准备，不在无环境时宣称 pass |
...
| `ai-judge-p37-ops-trust-monitoring-pack` | Ops read model 增加生产 artifact、公验、challenge、registry drift 监控 | 未开始 | 必须拆 helper，避免继续膨胀 `ops_read_model_pack.py` |
...
1. ai-judge-p37-public-verification-boundary-pack
2. ai-judge-p37-real-env-evidence-readiness-pack
3. ai-judge-p37-fairness-real-sample-benchmark-pack
