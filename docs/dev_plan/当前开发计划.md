...
| AI advisory shell | `ai_judge_service/app/applications/route_group_assistant.py` 已注册 `/internal/judge/apps/npc-coach/sessions/{session_id}/advice` 与 `/internal/judge/apps/room-qa/sessions/{session_id}/answer`；`assistant_agent_routes.py` 已构建 advisory context、capability boundary、Room Context Snapshot / Stage Summary 与官方裁决字段 sanitizer；`agent_runtime.py` 已支持本地 deterministic placeholder 开关 | AI internal shell、chat proxy、frontend read model、room context snapshot contract、deterministic placeholder、Ops-only calibration decision action bridge 与 P42 local reference regression 已完成；下一步推进热点拆分 |
...
| Ops calibration decision bridge | `chat/chat_server/src/models/judge/calibration_decision_ops_proxy.rs` 已新增 Ops-only AI decision log proxy；`debate_ops.rs` 注册 `/api/debate/ops/judge-calibration-decisions`，复用 `judge_review` RBAC、user/ip 限流、Idempotency-Key 与 no-secret 合同校验；`frontend/packages/ops-domain/src/runtimeReadiness.ts` 与 `OpsConsolePage.tsx` 已把 Fairness Calibration recommended actions 映射为 accept/reject/defer/request_more_evidence 操作 | P42 8.6 已完成；该操作只进入 Ops Console，不进入普通 Debate Room，不写 official verdict plane |
| 热点文件 | 当前 `app_factory.py` 约 2346 行、`ops_read_model_pack.py` 约 2463 行、`debate_ops.rs` 约 4989 行、`OpsConsolePage.tsx` 约 1575 行；frontend assistant UI 已拆到 `DebateAssistantPanel.tsx`，Ops decision proxy 已落在独立 model helper | P42 必须继续控制热点：下一步 route hotspot split 优先走 helper/domain/component，不继续膨胀热点文件 |
...
1. 执行 ai-judge-p42-route-hotspot-split-pack，继续控制 P42 新增代理与 UI 热点
2. 准备 P42 阶段收口前的本地参考证据刷新与完成/债务归档
...
| Batch-G | `hotspot-split-regression-closure` | 拆分热点、刷新本地参考证据并阶段收口 | 进行中（本地参考证据已刷新；热点拆分与阶段收口待执行） |
...
3. `chat/chat_server/src/handlers/debate_ops.rs`：约 4989 行。
4. `frontend/packages/app-shell/src/pages/OpsConsolePage.tsx`：约 1575 行。
...
1. 执行 `ai-judge-p42-route-hotspot-split-pack`，继续收敛 P42 新增代理、domain 与 UI 热点。
2. 完成热点拆分后，再准备 `ai-judge-p42-stage-closure-execute` 的本地参考证据刷新、完成快照与后置债务归档。
