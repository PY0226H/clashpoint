...
当前状态：进行中（`ai-judge-cases-hard-cut` 已完成，`ai-judge-judge-core-unification` phase1 已完成）
...
| `ai-judge-judge-core-unification` | 单核编排与状态机收敛 | 进行中（phase1 已完成） | 已落地 `JudgeCoreOrchestrator` 并接管 case/phase/final/review 状态推进，待继续收敛 replay/review 事件语义与跨层消费 |
...
1. `ai-judge-judge-core-unification`（phase2：replay/review 事件语义统一 + 对外 case 视图补齐 stage 展示）
...
| M2：Judge Core 完整化 | 进行中（phase1 已完成） | 已新增 `JudgeCoreOrchestrator` 并接管核心状态推进，待完成 replay/review 的统一 stage 收敛与接口展示 |
