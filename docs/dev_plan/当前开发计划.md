...
当前状态：执行中（P41 runtime readiness contract / chat proxy / Ops Console / calibration decision log / panel shadow candidate contract / release readiness ops evidence 已落地；下一步进入 route hotspot split）
...
1. ai-judge-p41-route-hotspot-split-pack
2. ai-judge-p41-local-reference-regression-pack
3. ai-judge-p41-stage-closure-execute
...
1. 先执行 `ai-judge-p41-route-hotspot-split-pack`，把 P41 继续增长的 projection/resolver/helper 从热点文件下沉出来。
2. 再执行 `ai-judge-p41-local-reference-regression-pack`，刷新 P41 本地参考证据并准备阶段收口输入。
3. 然后执行 `ai-judge-p41-stage-closure-execute`，将主体完成项与真实环境后置项按 stage closure 规则归档。
