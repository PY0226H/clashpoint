...
| `P2 Judge 主链迁移与领域收敛` | 将 phase/final 主链迁入新分层与领域对象 | 未开始 | 待 P1 完成后进入 |
...
1. `ai-judge-p1-runtime-migration`：将 phase/final 主链状态流转迁移到 `WorkflowOrchestrator + PostgresWorkflowStore`
2. `ai-judge-p1-trace-fact-bridge`：把 `trace_store` 的 `dispatch_receipt/replay/audit_alert` 读写逐步切到新 `JudgeFactRepository`
3. `ai-judge-p1-gateway-bootstrap`：落地 `LLM Gateway / Knowledge Gateway` 初版并给出首个接入点
### 模块完成同步历史
