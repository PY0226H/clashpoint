...
当前状态：进行中（P42 8.1 assistant advisory contract freeze 与 8.2 chat_server 用户侧 assistant advisory 代理已完成；下一步执行前端 Debate Room read model / UI 接入，继续保持 advisory-only，不写官方裁决链）
...
| AI advisory shell | `ai_judge_service/app/applications/route_group_assistant.py` 已注册 `/internal/judge/apps/npc-coach/sessions/{session_id}/advice` 与 `/internal/judge/apps/room-qa/sessions/{session_id}/answer`；`assistant_agent_routes.py` 已构建 advisory context、capability boundary 与官方裁决字段 sanitizer；`assistant_advisory_contract_v1` 已冻结 | AI internal shell 与合同已完成；chat proxy 已消费该合同，下一步让前端消费 chat read model |
...
| AI 请求 DTO | `ai_judge_service/app/models.py` 已有 `NpcCoachAdviceRequest` 与 `RoomQaAnswerRequest`，均 `extra=forbid`，包含 `trace_id`、`query/question`、可选 `case_id`；chat_server 已新增对应 public DTO | 请求/响应合同已在 AI 与 chat 两侧冻结，frontend 下一步复用 chat DTO 语义 |
...
| `ai-judge-p42-client-assistant-advisory-read-model-pack` | 前端 Debate Room 接入 advisory read model | 下一步 | `debate-domain` typed API、页面入口、not_ready 空态、advisory-only 文案与错误态测试 |
...
1. 执行 ai-judge-p42-client-assistant-advisory-read-model-pack，补 Debate Room typed API 与 not_ready 空态
2. 执行 ai-judge-p42-room-context-snapshot-contract-pack，稳定前端可解释上下文字段
...
| Batch-C | `frontend-read-model` | Debate Room 增加 assistant advisory 入口与 domain typed API，清晰展示辅助性质和未启用状态 | 下一步 |
...
1. 先执行 `ai-judge-p42-client-assistant-advisory-read-model-pack`，让 Debate Room 通过 chat proxy 消费 `not_ready/advisory` read model。
2. 再推进 `ai-judge-p42-room-context-snapshot-contract-pack` 与 `ai-judge-p42-advisory-runtime-placeholder-pack`，保持只读、不可越权。
