...
1. 在真实对象存储窗口执行 `artifact_store_healthcheck.py --enable-roundtrip --fail-on-not-ready`，保留 `productionReady=true` evidence。
2. 使用 `ai_judge_real_env_window_closure.sh --preflight-only` 校验真实样本 manifest、provider、callback、benchmark/fairness/runtime ops targets 全部 ready。
3. `preflight_ready` 后执行 `ai-judge-real-env-pass-window-execute-on-env`；`NPC Coach` / `Room QA` 保持暂停。
