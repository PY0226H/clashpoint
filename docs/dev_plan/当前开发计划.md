...
1. `ai-judge-p1-trace-fact-bridge`：将 `trace_store` 的 `dispatch_receipt/replay/audit_alert` 读写桥接到 `JudgeFactRepository`
2. `ai-judge-p1-runtime-failure-coverage`：补齐 blindization / contract-blocked 等分支的 workflow 失败事件覆盖与用例
3. `ai-judge-p1-gateway-bootstrap`：落地 `LLM Gateway / Knowledge Gateway` 初版并接入首条链路
### 模块完成同步历史
