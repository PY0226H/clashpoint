...
当前状态：开发中（已完成 P43 Batch-A `executor-config-policy`；下一步进入 NPC Coach / Room QA prompt bundle 与安全输出 schema；真实环境 pass 继续阻塞）
...
| AI advisory contract | [assistant_advisory_contract.py](/Users/panyihang/Documents/EchoIsle/ai_judge_service/app/applications/assistant_advisory_contract.py) 已冻结 `assistant_advisory_contract_v1`、顶层 key、Room Context Snapshot、Stage Summary、advisory-only boundary、forbidden official output fail-closed | P43 必须复用该合同，不新增官方裁决字段；真实 LLM 输出只能进入 `output` 中的安全 advisory 字段 |
| AI route | [assistant_agent_routes.py](/Users/panyihang/Documents/EchoIsle/ai_judge_service/app/applications/assistant_agent_routes.py) 已构建 shared context、knowledge gateway trace snapshot、policy isolation 与 response validator | P43 可在 route 层继续只调用 `execute_agent`，executor 内部负责 LLM 调用和结构化输出 |
...
| `ai-judge-p43-assistant-prompt-output-contract-pack` | 定义 NPC Coach / Room QA prompt bundle 与安全输出 schema | 待执行 | 输出仅允许 `safeGuidanceSummary`、`suggestedNextQuestions`、`contextCaveats` 等 advisory 字段，不允许胜负、评分、裁决理由或内部 trace |
...
1. 执行 `ai-judge-p43-assistant-prompt-output-contract-pack`。
2. 紧接着执行 `ai-judge-p43-assistant-llm-gateway-executor-pack`，基于 Batch-A 的 `llm_canary` settings 接入 fake / real gateway executor。
...
| Batch-B | `prompt-output-contract` | 为 NPC Coach / Room QA 建立 prompt builder、结构化输出 schema 与 forbidden output guard | 待执行 |
