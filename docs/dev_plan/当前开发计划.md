...
当前状态：进行中（`ai-judge-cases-hard-cut` 已完成，`ai-judge-judge-core-unification` 已完成，`ai-judge-contract-hardening` phase1+phase2 已完成，`ai-judge-cross-layer-sync` phase1 已完成）
...
| `ai-judge-cross-layer-sync` | chat_server 与上层调用方同步 | 进行中（phase1 已完成） | 已完成 callback 路由 `jobs -> cases` 与 dispatch request/response `job_id/jobId -> case_id/caseId` 硬切，待继续同步 replay/ops 视图语义 |

### 下一开发模块建议

1. `ai-judge-cross-layer-sync`（phase2：ops/replay 语义 `jobId -> caseId`）
2. `ai-judge-case-evidence-view`
3. `ai-judge-runtime-ops-pack`
...
1. 启动 `ai-judge-cross-layer-sync`，完成 `chat_server` 消费模型/OpenAPI/调用方字段同步硬切。
2. 进入 `ai-judge-case-evidence-view`，把 claim/policy/attestation/fairness 聚合到 case 详情主视图。
