...
当前状态：执行中（P41 runtime readiness contract / chat proxy / Ops Console / calibration decision log 已落地；下一步进入 panel shadow candidate contract）
...
| 前端/SDK | `debate-domain` 与 `DebateRoomPage` 已接入 public verify/challenge；`ops-domain` 与 `OpsConsolePage` 已展示 judge challenge queue、runtime readiness 与 calibration decision summary | 首批 readiness 首屏与 decision summary 已完成；下一步补 candidate readiness 与 stage closure evidence 展示 |
...
1. ai-judge-p41-panel-shadow-candidate-contract-pack
2. ai-judge-p41-release-readiness-ops-evidence-pack
3. ai-judge-p41-route-hotspot-split-pack
