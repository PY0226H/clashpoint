...
| AI advisory shell | `ai_judge_service/app/applications/route_group_assistant.py` 已注册 `/internal/judge/apps/npc-coach/sessions/{session_id}/advice` 与 `/internal/judge/apps/room-qa/sessions/{session_id}/answer`；`assistant_agent_routes.py` 已构建 advisory context、capability boundary、Room Context Snapshot / Stage Summary 与官方裁决字段 sanitizer；`assistant_advisory_contract_v1` 已冻结 | AI internal shell、chat proxy、frontend read model 与 room context snapshot contract 已完成；下一步进入 deterministic advisory placeholder |
...
| AI 请求 DTO | `ai_judge_service/app/models.py` 已有 `NpcCoachAdviceRequest` 与 `RoomQaAnswerRequest`，均 `extra=forbid`，包含 `trace_id`、`query/question`、可选 `case_id`；chat_server 已新增对应 public DTO；`frontend/packages/debate-domain/src/index.ts` 已新增 typed API 与前端合同 guard | 请求/响应、shared context 与 stage summary 合同已在 AI、chat 与 frontend 三层冻结；下一步补 deterministic placeholder 输出合同 |
...
| 热点文件 | 当前 `app_factory.py` 约 2346 行、`ops_read_model_pack.py` 约 2463 行、`debate_ops.rs` 约 4819 行、`OpsConsolePage.tsx` 约 1446 行；frontend assistant UI 已拆到 `DebateAssistantPanel.tsx` | P42 必须继续控制热点：后续 placeholder 也优先走 helper/domain/component，不继续膨胀热点文件 |
...
1. 执行 ai-judge-p42-advisory-runtime-placeholder-pack，补 deterministic advisory placeholder
2. 执行 ai-judge-p42-local-reference-regression-pack，刷新 P42 本地参考证据
...
1. 先执行 `ai-judge-p42-advisory-runtime-placeholder-pack`，在不启用真实 LLM 的前提下补 deterministic advisory placeholder。
2. 再执行 `ai-judge-p42-local-reference-regression-pack`，刷新 P42 本地参考证据，保持 `local_reference_ready` / `env_blocked` 口径。
