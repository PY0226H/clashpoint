...
当前状态：已完成 P0-A 官方裁决合同回归补强；下一步进入 P0-B real-env evidence drift guard；真实环境未提供，real-env pass 后置；`NPC Coach` / `Room QA` 继续暂停
...
1. 继续 P0-B ai-judge-real-env-evidence-drift-guard-pack，保护 local_reference / real-env pass 边界
2. 随后执行 P1-C ai-judge-runtime-ops-readiness-consistency-pack，对齐 AI/chat/frontend readiness 展示
3. 再执行 P1-D ai-judge-local-reference-regression-refresh-pack，刷新本地参考证据
...
2. 下一步执行 `ai-judge-real-env-evidence-drift-guard-pack`，保护 `local_reference` / `pass` 边界。
3. 接着执行 `ai-judge-runtime-ops-readiness-consistency-pack`，检查 AI、chat、frontend 三层 readiness 展示一致性。
4. 然后执行 `ai-judge-local-reference-regression-refresh-pack`，刷新本地参考证据。
5. 最后视前四项结果决定是否进入 `ai-judge-route-hotspot-map-audit-pack` 或 stage closure。
