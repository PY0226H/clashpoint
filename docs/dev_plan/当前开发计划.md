...
当前状态：执行中（P41 runtime readiness contract / chat proxy / Ops Console / calibration decision log / panel shadow candidate contract 已落地；下一步进入 release readiness ops evidence）
...
| chat_server 门面 | 已有 `/api/debate/sessions/{id}/judge-report/public-verify`、`/challenge`、`/challenge/request`、`/api/debate/ops/judge-challenge-queue` 与 `/api/debate/ops/judge-runtime-readiness` | runtime readiness 已接入主业务 Ops 门面；panel candidate summary 通过 runtime readiness 白名单投影给 Ops |
| 前端/SDK | `debate-domain` 与 `DebateRoomPage` 已接入 public verify/challenge；`ops-domain` 与 `OpsConsolePage` 已展示 judge challenge queue、runtime readiness、calibration decision summary 与 panel candidate/blocker 摘要 | 首批 readiness 首屏、decision summary 与 candidate readiness 摘要已完成；下一步补 release/stage closure evidence 展示 |
...
1. ai-judge-p41-release-readiness-ops-evidence-pack
2. ai-judge-p41-route-hotspot-split-pack
3. ai-judge-p41-local-reference-regression-pack
...
1. 先执行 `ai-judge-p41-release-readiness-ops-evidence-pack`，把 runtime readiness proxy、calibration decision log 与 panel candidate contract 纳入 release readiness artifact / stage closure evidence。
2. 再执行 `ai-judge-p41-route-hotspot-split-pack`，把 P41 继续增长的 projection/resolver/helper 从热点文件下沉出来。
3. 然后执行 `ai-judge-p41-local-reference-regression-pack`，刷新 P41 本地参考证据并准备阶段收口输入。
