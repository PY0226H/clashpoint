...
当前状态：已完成 P0-A 官方裁决合同回归补强、P0-B real-env evidence drift guard 与 P1-C readiness 三层一致性补强；下一步进入 P1-D 本地参考回归证据刷新；真实环境未提供，real-env pass 后置；`NPC Coach` / `Room QA` 继续暂停
...
1. 进入 P1-D ai-judge-local-reference-regression-refresh-pack，刷新本地参考证据
2. 随后视结果决定是否执行 P2-E route hotspot map audit
3. 若本地参考证据稳定，可准备 stage closure
...
4. 下一步执行 `ai-judge-local-reference-regression-refresh-pack`，刷新本地参考证据。
5. 最后视 P1-D 结果决定是否进入 `ai-judge-route-hotspot-map-audit-pack` 或 stage closure。
