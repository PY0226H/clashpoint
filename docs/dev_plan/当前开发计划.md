...
当前状态：进行中（`ai-judge-cases-hard-cut` phase1 已完成）
...
| 阶段 | 目标 | 状态 | 说明 |
| --- | --- | --- | --- |
| `ai-judge-cases-hard-cut` | 外部 `job` 语义硬切为 `case` | 进行中（phase1 已完成） | 请求/回调/路由/响应主键已切换，待补 case 聚合创建与详情主入口 |
...
1. `ai-judge-cases-hard-cut`（phase2：补 `POST /internal/judge/cases` 与 `GET /internal/judge/cases/{case_id}`）
2. `ai-judge-judge-core-unification`
3. `ai-judge-contract-hardening`
...
| 里程碑 | 状态 | 说明 |
| --- | --- | --- |
| M1：cases 硬切完成 | 进行中（phase1 已完成） | 已完成外部契约与路由 `job -> case` 硬切；待补 `POST /internal/judge/cases` 与 `GET /internal/judge/cases/{case_id}` 聚合视图 |
