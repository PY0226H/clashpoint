...
当前状态：进行中（P42 8.1 assistant advisory contract freeze 已完成；下一步执行 chat_server 用户侧 assistant advisory 代理，保持 advisory-only，不写官方裁决链）
...
| AI advisory shell | `ai_judge_service/app/applications/route_group_assistant.py` 已注册 `/internal/judge/apps/npc-coach/sessions/{session_id}/advice` 与 `/internal/judge/apps/room-qa/sessions/{session_id}/answer`；`assistant_agent_routes.py` 已构建 advisory context、capability boundary 与官方裁决字段 sanitizer | AI internal shell 已有，下一步应冻结对外安全合同，并让 chat 代理消费 |
...
| AI 请求 DTO | `ai_judge_service/app/models.py` 已有 `NpcCoachAdviceRequest` 与 `RoomQaAnswerRequest`，均 `extra=forbid`，包含 `trace_id`、`query/question`、可选 `case_id` | DTO 起点存在；需要把响应合同、错误语义、可见字段白名单冻结 |
| chat 代理 | `chat/chat_server/src/config.rs` 的 `AiJudgeConfig` 目前已有 public verify / challenge / runtime readiness 路径，但没有 npc coach / room qa 路径；`debate_ops.rs` 仅有 runtime readiness 与 challenge queue 代理；用户侧 `debate_judge.rs` 已有 public verify / challenge 代理 | P42 需要新增主业务用户侧 assistant advisory 代理，并补 config / model / route / OpenAPI / 测试 |
| frontend 消费 | `frontend/packages/debate-domain/src/index.ts` 已有 public verify / challenge typed API；`frontend/packages/ops-domain/src/runtimeReadiness.ts` 已有 runtime readiness typed API；尚未发现 NPC Coach / Room QA typed read model 或页面入口 | P42 需要在 `debate-domain` + `DebateRoomPage` 做轻量产品入口，不能塞入 Ops Console |
| 热点文件 | 当前 `app_factory.py` 约 2346 行、`ops_read_model_pack.py` 约 2463 行、`debate_ops.rs` 约 4819 行、`OpsConsolePage.tsx` 约 1446 行 | P42 必须继续控制热点：新增 chat proxy/helper 与 frontend domain 文件，不把逻辑继续堆进热点文件 |
...
| `ai-judge-p42-chat-assistant-advisory-proxy-pack` | 将 assistant advisory 通过 chat_server 主业务门面代理 | 待执行 | config paths、request/response DTO、session permission、rate limit、proxy no-secret guard、OpenAPI、route/model tests |
| `ai-judge-p42-client-assistant-advisory-read-model-pack` | 前端 Debate Room 接入 advisory read model | 待执行 | `debate-domain` typed API、页面入口、not_ready 空态、advisory-only 文案与错误态测试 |
| `ai-judge-p42-room-context-snapshot-contract-pack` | 稳定 Room Context / Stage Summary 只读合同 | 待执行 | AI shared context 字段白名单、chat case/session id 映射、frontend 可解释状态映射 |
...
1. 执行 ai-judge-p42-chat-assistant-advisory-proxy-pack，新增 chat_server 用户侧 assistant advisory 代理
2. 执行 ai-judge-p42-client-assistant-advisory-read-model-pack，补 Debate Room typed API 与 not_ready 空态
...
| Batch-B | `chat-product-proxy` | 在 chat_server 增加用户侧 NPC Coach / Room QA 代理，完成权限、限流、OpenAPI 与 proxy no-secret guard | 待执行 |
| Batch-C | `frontend-read-model` | Debate Room 增加 assistant advisory 入口与 domain typed API，清晰展示辅助性质和未启用状态 | 待执行 |
| Batch-D | `room-context-contract` | 稳定 shared room context / stage summary / case id 映射，不暴露官方裁决私有字段 | 待执行 |
...
1. Web/Desktop 共用 `debate-domain`，不在双端重复业务逻辑。
2. Debate Room 能调用 chat proxy 并展示安全 read model。
3. 所有 UI 输出都保持 advisory-only，不改变 judge report 主流程。
...
1. 先执行 `ai-judge-p42-chat-assistant-advisory-proxy-pack`，建立 chat_server 用户侧门面。
2. 再执行 `ai-judge-p42-client-assistant-advisory-read-model-pack`，让 Debate Room 能消费 `not_ready/advisory` read model。
3. 后续推进 `ai-judge-p42-room-context-snapshot-contract-pack` 与 `ai-judge-p42-advisory-runtime-placeholder-pack`，保持只读、不可越权。
