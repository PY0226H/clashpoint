...
| AI advisory shell | `ai_judge_service/app/applications/route_group_assistant.py` 已注册 `/internal/judge/apps/npc-coach/sessions/{session_id}/advice` 与 `/internal/judge/apps/room-qa/sessions/{session_id}/answer`；`assistant_agent_routes.py` 已构建 advisory context、capability boundary、Room Context Snapshot / Stage Summary 与官方裁决字段 sanitizer；`agent_runtime.py` 已支持本地 deterministic placeholder 开关 | AI internal shell、chat proxy、frontend read model、room context snapshot contract 与 deterministic placeholder 已完成；下一步刷新 P42 local reference regression |
...
1. 执行 ai-judge-p42-local-reference-regression-pack，刷新 P42 本地参考证据
2. 评估 ai-judge-p42-calibration-decision-ops-action-bridge-pack 是否进入 Ops-only 操作面
...
1. 先执行 `ai-judge-p42-local-reference-regression-pack`，刷新 P42 本地参考证据，保持 `local_reference_ready` / `env_blocked` 口径。
2. 再评估 `ai-judge-p42-calibration-decision-ops-action-bridge-pack` 是否进入 Ops-only 操作面。
3. 后续按需推进 `ai-judge-p42-route-hotspot-split-pack` 与 P42 阶段收口。
