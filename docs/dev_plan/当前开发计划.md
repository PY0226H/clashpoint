...
1. `ai-judge-runtime-ops-pack`（phase2：与 stage closure 文档联动自动回填）
2. `ai-judge-real-env-window-closure`（真实环境窗口可用时执行）
3. `ai-judge-stage-closure-execute`（阶段结束时执行正式收口）
...
| M4：收口与证据归档 | 进行中 | 已完成 runtime ops pack phase1 + doc governance refresh + stage closure evidence phase1；待完成 phase2 自动回填 |

---

## 9. 下一步（执行入口）

1. 推进 `ai-judge-runtime-ops-pack`（phase2），将收口草案和 runtime 收口包做自动回填联动。
2. 在真实环境窗口可用时推进 `ai-judge-real-env-window-closure`，补齐 real-env 证据冻结结论。
3. 阶段结束时执行 `ai-judge-stage-closure-execute`，完成长期文档回填与活动计划归档。
