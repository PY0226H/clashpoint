...
1. `ai-judge-real-env-window-closure`（真实环境窗口可用时执行）
2. `ai-judge-stage-closure-execute`（阶段结束时执行正式收口）
3. `ai-judge-next-iteration-planning`（进入下一迭代规划）
...
| M4：收口与证据归档 | 进行中 | 已完成 runtime ops pack phase1+phase2 + doc governance refresh + stage closure evidence phase1；待完成真实环境窗口收口与阶段执行归档 |

---

## 9. 下一步（执行入口）

1. 在真实环境窗口可用时推进 `ai-judge-real-env-window-closure`，补齐 real-env 证据冻结结论。
2. 阶段结束时执行 `ai-judge-stage-closure-execute`，完成长期文档回填与活动计划归档。
3. 启动 `ai-judge-next-iteration-planning`，定义下一阶段目标与模块顺序。
