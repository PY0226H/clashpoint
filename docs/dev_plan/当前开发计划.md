...
| M2：Agent Runtime MVP | 进行中 | Judge runtime 已切为 Courtroom Role MVP，phase/final/replay 已写入 `judgeTrace.agentRuntime/courtroomRoles` |
| M3：Ledger 驱动裁决与公平强化 | 未开始 | claim/evidence/verdict/fairness/opinion 一体化 |
...
| `ai-judge-agent-role-runtime-mvp` | 角色化 Agent 主链 | 进行中 | Judge executor 已升级为可执行角色运行时，phase/final/replay 主链已写入角色追踪；下一步继续收敛到 registry 编排 |
| `ai-judge-registry-runtime` | Prompt/Tool/Policy Registry | 未开始 | 待执行 |
| `ai-judge-claim-evidence-ledger-v2` | Claim/Evidence Ledger 升级 | 未开始 | 待执行 |
| `ai-judge-panel-arbiter-opinion-v2` | Panel/Chief/Opinion 升级 | 未开始 | 待执行 |
...
1. `ai-judge-registry-runtime`
2. `ai-judge-claim-evidence-ledger-v2`
3. `ai-judge-panel-arbiter-opinion-v2`
