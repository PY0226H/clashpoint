...
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| virtual-judge-npc-kafka-consumer-cutover | `virtual-judge-npc-event-consumption-loop` | 可靠性 | MVP 为了本地闭环先使用 `npc_service` webhook trigger 接收 `DebateMessageCreated`；正式多实例运行前需要由 Kafka/event-bus consumer 接管 ingestion，避免 webhook 临时路径长期存在。 | NPC MVP 进入多服务常驻联调、预发布或需要多实例部署前 | Kafka/event-bus consumer 成为唯一事件摄入入口；webhook trigger 移除；offset、重试、DLQ、拒绝不无限重试、服务不可用不阻塞发言等语义均有测试证据。 | `npc_service` event consumer integration tests、chat NPC action targeted tests、Kafka/DLQ 故障注入、本地 runtime smoke 与 `post-module-test-guard --mode full`。 |
| virtual-judge-npc-runtime-browser-smoke-evidence | `virtual-judge-npc-e2e-smoke-and-fallback-hardening` | 环境依赖 | 本轮已有可重复 in-process full smoke；真实 chat/notify/frontend/npc_service 多服务浏览器 smoke 需要稳定本地运行窗口和服务编排，不作为 MVP 主体闭环阻塞项。 | 预发布验收、前端交互验收或本地 runtime stack 稳定后 | 启动 chat、notify、frontend、npc_service；用户发送辩论发言后前端可见 NPC action；断线重连可 replay；LLM 失败和 `npc_service` 停止不影响用户发言。 | Playwright / web smoke 截图与服务日志；必要时执行 `pnpm --dir frontend e2e:smoke:web` 并归档证据。 |
| virtual-judge-npc-observability-dashboard-baseline | `virtual-judge-npc-event-consumption-loop` | 可观测性 | 当前没有生产/预发布流量，测试已覆盖主语义，但尚未建立 NPC 决策状态、fallback、callback、DLQ 和延迟的 dashboard/告警基线。 | 预发布 readiness、运营观察面板建设或真实 provider canary 前 | 指标与日志覆盖 decision status、executor、fallback reason、callback accepted/rejected/failed、DLQ size、latency、LLM timeout/rate-limit；dashboard 与告警阈值可审核。 | dashboard export、fault injection 证据、日志采样与运行态告警演练。 |
| virtual-judge-npc-pause-state-machine-evaluation | `virtual-judge-npc-llm-agent-loop-mvp-stage-closure` | 工程债 / 产品边界 | 暂停/恢复辩论是高影响房间状态机，不属于当前娱乐向公开发言 MVP；在状态机、权限、审计、replay 语义未定档前不接入 NPC 工具列表。 | 产品确认 NPC 可触发暂停/恢复，且房间 pause/resume 状态机设计完成后 | 完成 pause/resume PRD 与系统设计；NPC tool schema、权限、审计、幂等、replay 与降级语义明确；chat/notify/frontend/npc_service 跨层测试覆盖。 | 状态机 targeted tests、NPC tool guard tests、WS replay tests、前端 smoke 与阶段收口文档。 |
