...
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| virtual-judge-npc-beta-real-env-canary-dashboard-closure | `virtual-judge-npc-observability-runtime-smoke` / `virtual-judge-npc-beta-readiness-productization-stage-closure` | 环境依赖 / 可观测性 | P3-I 已建立仓内可复验 smoke 与观测基线，但当前没有 Beta/staging 真实运行窗口；不得把本地 mock Web smoke、local fallback 或未接真实 OpenAI/Kafka/dashboard 的证据宣称为真实环境通过。 | Beta/staging 环境、真实 OpenAI-compatible provider、Kafka/event-bus topic、日志/指标聚合面板具备后 | 在真实部署环境完成 NPC canary：真实用户发言触发 NPC action、观战实时可见、断线 replay 不丢不重、LLM 失败 fallback、`npc_service` 停止不阻塞发言；dashboard 能查询 decision status、executor、fallback reason、callback accepted/rejected/failed、DLQ、latency、provider 错误码、成本与熔断。 | 真实环境 Playwright / 手工 smoke 记录、服务日志、Kafka offset / DLQ 证据、OpenAI canary 成本/延迟摘要、dashboard export 与告警演练记录。 |
| virtual-judge-npc-pause-suggestion-implementation-decision | `virtual-judge-npc-pause-state-machine-design-gate` / `virtual-judge-npc-beta-readiness-productization-stage-closure` | 工程债 / 产品边界 | P3-H 已完成暂停/恢复状态机设计门禁，并明确强暂停/恢复必须由 `chat` 状态机和审批边界执行；当前产品尚未确认进入实现，避免把娱乐向 NPC 直接升级为高影响房间状态控制者。 | 产品确认需要 NPC 暂停能力的首个实现切片后 | 先落地 `pause_suggestion` 最小闭环：NPC 只能公开建议暂停，不改变房间状态；若后续进入 `soft_pause/hard_pause/resume`，必须新增审批、审计、幂等、倒计时、WS replay、输入禁用和恢复语义，并由 `chat` 状态机执行。 | `chat` pause suggestion / room state targeted tests、npc_service tool guard tests、notify WS replay tests、前端 smoke 与阶段收口文档。 |
