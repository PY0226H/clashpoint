...
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| virtual-judge-npc-beta-real-env-canary-dashboard-closure | `virtual-judge-npc-real-env-and-pause-suggestion-stage-closure` | 环境依赖 / 可观测性 | 仓内 smoke / guard 已通过，但当前没有 Beta/staging 真实运行窗口；不得把本地 mock Web smoke、local fallback 或未接真实 OpenAI/Kafka/dashboard 的证据宣称为真实环境通过。 | Beta/staging 环境、真实 OpenAI-compatible provider、Kafka/event-bus topic、日志/指标聚合面板具备后 | 在真实部署环境完成 NPC canary：真实用户发言触发 NPC action、观战实时可见、断线 replay 不丢不重、LLM 失败 fallback、`npc_service` 停止不阻塞发言；dashboard 能查询 decision status、executor、fallback reason、callback accepted/rejected/failed、DLQ、latency、provider 错误码、成本与熔断。 | 真实环境 Playwright / 手工 smoke 记录、服务日志、Kafka offset / DLQ 证据、OpenAI canary 成本/延迟摘要、dashboard export 与告警演练记录。 |
| virtual-judge-npc-strong-pause-approval-state-machine-implementation | `virtual-judge-npc-real-env-and-pause-suggestion-stage-closure` | 工程债 / 产品边界 | `pause_suggestion` 最小闭环已完成；`soft_pause/hard_pause/resume` 属于高影响房间状态控制，不适合在娱乐向 NPC 中直接开放，必须等产品确认、审批边界和真实环境证据齐备后再做。 | 产品明确确认需要强暂停 / 恢复能力，且 Beta/staging 真实 canary 与观测面板已具备后 | 由 `chat` 状态机承载强暂停 / 恢复，补齐审批、审计、幂等、倒计时、WS replay、输入禁用、恢复语义、观战一致性、Ops 回滚与正式裁决隔离；NPC 只能提出候选或建议，不直接绕过审批执行强状态变化。 | `chat` pause state machine targeted tests、npc_service tool guard tests、notify WS replay tests、前端参赛/观战 smoke、Ops 审批回归、阶段收口文档。 |
