...
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| virtual-judge-npc-beta-real-env-canary-dashboard-closure | `virtual-judge-npc-beta-real-env-no-go-stage-closure` | 环境依赖 / 可观测性 | 仓内 smoke / guard 已通过，P1-B/P1-C/P2-G 已完成 readiness 证据化并判定 `env_blocked / no-go`；当前仍没有 Beta/staging 真实运行窗口、真实 provider、Kafka/topic/group、dashboard/log 权限、canary session、测试账号和 Ops 回滚 owner。不得把本地 mock、local fallback、字段存在或未接真实 OpenAI/Kafka/dashboard 的证据宣称为真实环境通过。 | Beta/staging 环境、真实 OpenAI-compatible provider、Kafka/event-bus topic、日志/指标聚合面板、canary session、参赛/观战账号、Ops 权限和回滚 owner 均具备后 | 在真实部署环境完成 NPC canary：真实用户发言触发 NPC action、观战实时可见、断线 replay 不丢不重、LLM 失败 fallback、`npc_service` 停止不阻塞发言；dashboard 能查询 decision status、executor、fallback reason、callback accepted/rejected/failed、DLQ、latency、provider 错误码、成本与熔断，并产出 `pass / fail / rollback_required` 证据。 | 真实环境 Playwright / 手工 smoke 记录、服务日志、Kafka offset / DLQ 证据、OpenAI canary 成本/延迟摘要、dashboard export 与告警演练记录。 |
