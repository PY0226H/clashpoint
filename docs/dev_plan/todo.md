...
### C37. AI Judge 平台化重构阶段收口（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| `ai-judge-p33-real-env-pass-window-execute-on-env` | ai-judge-stage-closure-execute | 环境依赖 | 当前计划建议包含真实环境模块，需在环境窗口就绪后执行收口 | REAL_CALIBRATION_ENV_READY=true 且具备可用真实样本后 | 完成该模块并产出 real-env 证据工件，状态达到 pass | 执行对应模块脚本并归档 artifacts/harness 与 docs/loadtest/evidence |

### C38. AI Judge 平台化重构阶段收口（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
...
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| ai-judge-p37-real-env-pass-window-execute-on-env | `ai-judge-p37-local-reference-regression-pack` | 环境依赖 | P37 当前只有本地参考证据，真实样本、真实 provider/callback、生产对象存储与真实服务窗口尚不可用；本轮不得把 `local_reference_ready` 冒充为 real-env `pass`。 | `REAL_CALIBRATION_ENV_READY=true`、真实样本、真实服务窗口、生产对象存储与 P37 release readiness 输入同时具备后 | 重新运行 P37 runtime ops pack / fairness real sample benchmark / panel shadow / registry release gate / real-env closure 链路，输出真实环境 `pass` 工件，并按结果更新 completed/todo。 | `bash scripts/harness/ai_judge_runtime_ops_pack.sh --root /Users/panyihang/Documents/EchoIsle --evidence-dir docs/loadtest/evidence`（不带 `--allow-local-reference`）；必要时配套执行真实样本 benchmark、panel shadow 与 registry release gate 采样并归档到 `artifacts/harness` 与 `docs/loadtest/evidence`。 |

### C42. AI Judge 平台化重构阶段收口（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| `ai-judge-p37-real-env-pass-window-execute-on-env`（仅真实环境齐备后执行） | ai-judge-stage-closure-execute | 环境依赖 | 当前计划建议包含真实环境模块，需在环境窗口就绪后执行收口 | REAL_CALIBRATION_ENV_READY=true 且具备可用真实样本后 | 完成该模块并产出 real-env 证据工件，状态达到 pass | 执行对应模块脚本并归档 artifacts/harness 与 docs/loadtest/evidence |

### C43. AI Judge 平台化重构阶段收口（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| （无新增） | 当前延后项已写入技术债池 | （无） | （无） | （无） | （无） | （无） |

### C44. AI Judge 平台化重构阶段收口（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
...
### C46. AI Judge Real-Env Readiness Prep 后置技术债（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| ai-judge-official-real-env-pass-window-readiness-debt | `ai-judge-real-env-readiness-prep-stage-closure` | 环境依赖 | 当前没有真实环境；对象存储 healthcheck evidence 仍为 `local_reference` / `productionReady=false`，real-env preflight 仍为 `env_blocked`；不得把本地参考、mock provider/callback 或手工 ready 写成 real-env pass。 | `REAL_CALIBRATION_ENV_READY=true`，且真实样本 manifest、真实 provider、真实 callback、生产对象存储、benchmark/fairness/runtime ops targets 均具备可审核证据后 | 按 `ai_judge_real_env_readiness_inputs_checklist.md` 填写 readiness 输入；生产对象存储 healthcheck 输出 `productionReady=true`；`ai_judge_real_env_window_closure.sh --preflight-only` 输出 `preflight_ready`；随后执行真实环境 pass window 并产出 `pass` 证据，更新 completed/todo 与章节完成度映射。 | `/Users/panyihang/Documents/EchoIsle/ai_judge_service/.venv/bin/python ai_judge_service/scripts/artifact_store_healthcheck.py --enable-roundtrip --fail-on-not-ready --output docs/loadtest/evidence/ai_judge_artifact_store_healthcheck.json`；`bash scripts/harness/ai_judge_real_env_window_closure.sh --preflight-only`；真实窗口后运行对应 P5/runtime ops/stage closure evidence 链路并归档 `artifacts/harness` 与 `docs/loadtest/evidence`。 |

### C47. 虚拟裁判 NPC 后置技术债（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| virtual-judge-npc-beta-real-env-canary-dashboard-closure | `virtual-judge-npc-beta-real-env-no-go-stage-closure`, `virtual-judge-npc-real-env-input-unblock-stage-closure` | 环境依赖 / 可观测性 | 仓内 smoke / guard 已通过，readiness no-go 与输入解锁包均已收口；Owner 交付合同、Evidence 脱敏模板、Readiness Gate 重跑手册、目标环境地图、Kafka/Dashboard 合同、Canary Session 与账号输入包已定义，但当前仍没有 Beta/staging 真实运行窗口、真实 provider、Kafka/topic/group、dashboard/log 权限、canary session、测试账号、Ops 回滚 owner、目标环境 migration 通过证据和数据合规 reviewer。不得把本地 mock、local fallback、字段存在、输入模板存在或未接真实 OpenAI/Kafka/dashboard 的证据宣称为真实环境通过。 | Beta/staging 环境、真实 OpenAI-compatible provider、Kafka/event-bus topic、日志/指标聚合面板、canary session、参赛/观战账号、Ops 权限、回滚 owner、目标环境 migration 证据和数据合规 reviewer 均具备后 | 先按输入解锁包补齐脱敏证据并输出 `ready_to_rerun_gate`；再在真实部署环境完成 NPC canary：真实用户发言触发 NPC action、观战实时可见、断线 replay 不丢不重、LLM 失败 fallback、`npc_service` 停止不阻塞发言；dashboard 能查询 decision status、executor、fallback reason、callback accepted/rejected/failed、DLQ、latency、provider 错误码、成本与熔断，并产出 `pass / fail / rollback_required` 证据。 | 真实环境输入解锁 Decision、Playwright / 手工 smoke 记录、服务日志、Kafka offset / DLQ 证据、OpenAI canary 成本/延迟摘要、dashboard export 与告警演练记录。 |
| virtual-judge-npc-strong-pause-approval-state-machine-implementation | `virtual-judge-npc-beta-real-env-no-go-stage-closure` | 工程债 / 产品边界 | `pause_suggestion` 最小闭环已完成；`soft_pause/hard_pause/resume` 属于高影响房间状态控制，不适合在娱乐向 NPC 中直接开放。当前真实环境 release decision 仍为 `env_blocked / no-go`，必须等产品确认、审批边界和真实环境证据齐备后再做。 | 产品明确确认需要强暂停 / 恢复能力，且 Beta/staging 真实 canary 与观测面板已具备后 | 由 `chat` 状态机承载强暂停 / 恢复，补齐审批、审计、幂等、倒计时、WS replay、输入禁用、恢复语义、观战一致性、Ops 回滚与正式裁决隔离；NPC 只能提出候选或建议，不直接绕过审批执行强状态变化。 | `chat` pause state machine targeted tests、npc_service tool guard tests、notify WS replay tests、前端参赛/观战 smoke、Ops 审批回归、阶段收口文档。 |

### C48. Chat Server Redis 限流后置技术债（来源：当前开发计划）
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| chat-server-rate-limit-real-env-evidence-and-tuning | `chat-server-redis-rate-limit-gcra-stage-closure`, `chat-server-rate-limit-runtime-observability-stage-closure` | 环境依赖 / 真实流量调参 | GCRA、explicit burst、fallback、运行态 metrics、internal endpoint 与调参基线文档均已完成；当前没有 staging / Beta / production 真实运行窗口、真实流量样本、多实例指标汇总或 dashboard 证据。不得在缺少证据时继续调低 `limit/window/burst`，也不得把本地 metrics 快照宣称为 real-env tuning 通过。 | staging/Beta/production 真实环境、Redis 配置、internal key、测试账号/测试房间/测试订单、日志或 dashboard 权限、多实例采样入口和数据合规口径均具备后 | 按 `Redis限流优化_运行态调参与压测基线方案.md` 采集 before/after metrics，计算 scope 级 `rejectRate`、`nearLimitRate`、`fallbackRate`、`redisErrorRate`，输出 `pass/no_tuning_needed`、`needs_burst_tuning` 或 `rollback_required` 结论；如确需调参，优先只做启动时 burst override，并保留安全上限与回滚方案。 | internal metrics 原始 JSON、scope 派生指标摘要、压测命令脱敏记录、真实环境日志/dashboard 截图或 export、代表 route 429/header 回归；如改配置策略，补 `request_guard`、`redis_store`、OpenAPI/配置 schema 相关测试并运行合适门禁。 |
| chat-server-rate-limit-cross-service-policy-governance | `chat-server-redis-rate-limit-gcra-stage-closure` | 工程债 / 发布前增强 | 当前 `chat_server` 单体内 Redis Lua GCRA 足以支撑本地与近阶段部署；独立 rate-limit service、API Gateway 限流或统一跨服务策略表会显著扩大架构面，不适合在本轮提前引入。 | 服务拆分为多微服务、出现跨服务重复 scope 策略，或上线前架构评审明确需要统一入口限流时 | 形成跨服务限流策略 ADR，明确 Gateway / service-local / independent limiter 的职责边界、故障模式、配置来源、审计与测试策略；若 Go，落地统一策略表或服务边界。 | ADR 评审记录 + 配置 schema 测试 + 跨服务调用方回归 + 故障注入/降级演练证据。 |
