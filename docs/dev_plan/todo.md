...
| 债务项 | 来源模块 | 债务类型 | 当前不做原因 | 触发时机 | 完成定义（DoD） | 验证方式 |
|---|---|---|---|---|---|---|
| chat-server-rate-limit-runtime-config-and-real-traffic-tuning | `chat-server-redis-rate-limit-gcra-stage-closure` | 可观测性 / 性能压测 | GCRA 与 explicit burst 已落地，但当前仍缺运行态指标基线、真实流量样本和压测报告；现在继续调低 `limit/window/burst` 难以归因，容易误伤正常用户。 | 限流 metrics internal endpoint 落地后，或获得压测/真实流量样本后 | 基于 scope 维度的允许率、拒绝率、near-limit、fallback、Redis error 数据形成调参报告；必要时落地启动时 burst 配置覆盖，并保持安全上限校验。 | internal metrics 快照 + 压测报告归档到 `docs/loadtest/evidence/`；若改配置策略，补 `request_guard`、`redis_store` 与代表 route 回归。 |
