...
更新时间：2026-04-28
状态：已更新（对齐 P40 stage closure；P41 runtime readiness 与 calibration decision log 已落地）
...
| Adaptive Judge Platform（方案 Phase 3） | 部分落地（高） | gateway core、policy/prompt/tool registry、ops read model、registry release gate、release readiness artifact、panel runtime readiness、runtime ops closure backfill、chat/frontend runtime readiness 控制面与 calibration decision log 已推进；auto-calibration、多模型 panel 生产化与 domain-specific judge families 仍后置 |
...
`截至当前 P41 交付，AI Judge 已完成“制度化裁判庭 + 本地可信外部化 + public verification 产品面 + 受约束 challenge/review 用户与 Ops 基础桥接 + runtime readiness Ops 控制面首屏 + calibration recommendation 人工决策日志”。下一步从“能不能看见 readiness”转为“能不能冻结 panel shadow candidate 合同并把 release readiness evidence 收口”。真实环境 pass 与远期协议扩展继续后置。`
...
3. P41 已完成的控制面能力：
   ...
   - calibration advisor 的 recommended actions 已支持人工 decision log，写入 durable facts repository，并进入 advisor/runtime readiness summary；多模型 panel 仍是 shadow/readiness，不是生产切换。
...
1. 执行 `ai-judge-p41-panel-shadow-candidate-contract-pack`，冻结 candidate model/profile、成本、时延、drift、switch blockers 与 release gate 输入。
2. 执行 `ai-judge-p41-release-readiness-ops-evidence-pack`，把 P41 Ops 控制面证据纳入 release readiness artifact 与 stage closure evidence。
