...
更新时间：2026-04-28
状态：已更新（对齐 P40 stage closure；P41 首批 Fairness / Adaptive Runtime Ops Control Plane 已落地）
...
| Fairness Hardened（方案 Phase 2） | 基本落地（环境阻塞项除外） | Fairness gate、panel disagreement、local reference benchmark/freeze、真实样本 benchmark readiness、panel shadow evaluation、normalized fairness/panel evidence、citation verifier evidence gate、release gate 输入与 runtime readiness Ops 产品面已完成；真实样本实跑与真实环境 pass 仍待窗口 |
| Adaptive Judge Platform（方案 Phase 3） | 部分落地（高） | gateway core、policy/prompt/tool registry、ops read model、registry release gate、release readiness artifact、panel runtime readiness、runtime ops closure backfill 与 chat/frontend runtime readiness 控制面已推进；auto-calibration、多模型 panel 生产化与 domain-specific judge families 仍后置 |
...
`截至 P41 首批交付，AI Judge 已完成“制度化裁判庭 + 本地可信外部化 + public verification 产品面 + 受约束 challenge/review 用户与 Ops 基础桥接 + runtime readiness Ops 控制面首屏”。下一步从“能不能看见 readiness”转为“能不能对 calibration recommendation、panel shadow candidate 与 release readiness evidence 做人工决策和阶段收口”。真实环境 pass 与远期协议扩展继续后置。`
...
3. P41 首批已完成的控制面能力：
   ...
   - calibration advisor 仍偏建议读取，缺人工 decision log；多模型 panel 仍是 shadow/readiness，不是生产切换。
...
1. 执行 `ai-judge-p41-calibration-decision-log-pack`，为 calibration advisor 增加人工决策日志，保持不自动应用 policy/prompt/tool。
2. 执行 `ai-judge-p41-panel-shadow-candidate-contract-pack`，冻结 candidate model/profile、成本、时延、drift、switch blockers 与 release gate 输入。
3. 执行 `ai-judge-p41-release-readiness-ops-evidence-pack`，把 P41 Ops 控制面证据纳入 release readiness artifact 与 stage closure evidence。
4. 执行 `ai-judge-p41-route-hotspot-split-pack`，继续下沉 AI projection、chat proxy helper 与 frontend resolver，控制热点文件膨胀。
5. 执行 `ai-judge-p41-local-reference-regression-pack` 与 `ai-judge-p41-stage-closure-execute`，刷新本地参考证据并完成阶段收口。
6. 等真实环境具备后执行 `ai-judge-real-env-pass-window-execute-on-env` 或后续等价 real-env pass window，补齐真实环境 `pass` 证据；在此之前继续保持 `env_blocked`。
