...
当前主线：`virtual-judge-npc-beta-readiness-productization`（已收口）
...
| 前端展示 | [DebateNpcPanel.tsx](/Users/panyihang/Documents/EchoIsle/frontend/packages/app-shell/src/components/DebateNpcPanel.tsx) 与 [DebateNpcModel.ts](/Users/panyihang/Documents/EchoIsle/frontend/packages/app-shell/src/components/DebateNpcModel.ts) 已提供 NPC 面板、动态状态形象、公开呼叫和反馈入口 | 下一阶段如继续增强视觉资产，应保持与 Judge / Draw 区隔 |
...
| P4-J. `virtual-judge-npc-beta-stage-closure` | 阶段收口，回写 completed/todo 并归档计划 | 已完成 | 已回写 completed/todo，归档本活动计划，并将 default 当前计划重置为空档 |

## 6. 下一开发模块建议

1. 本阶段已完成 P4-J 收口。
2. 后续若继续推进虚拟裁判 NPC，应先补真实 Beta/staging 环境 canary 与 dashboard 证据，或按产品确认执行 `pause_suggestion` 最小闭环。
3. 暂停强动作实现不并入本阶段；如后续进入实现，必须先完成 `pause_suggestion` 最小闭环并继续保持强动作审批边界。
...
1. `DebateNpcPanel.tsx` 将状态文案升级为 `Watching / On mic / Cheering / Quiet / Manual / Offline`，并增加 `Live NPC` 边界标识。
2. NPC 动态形象从单一 avatar 扩展为裁判台式舞台：状态环、头像 band、表情、信号条和 effect burst。
3. `praise / effect` 使用高强度动效，`speak` 使用发言动效，`state_changed / silent / unavailable` 降低存在感。
4. CSS 增加多状态色、动作强度、移动端稳定栅格和 `prefers-reduced-motion` 降级。
5. 组件测试补充 `Live NPC`、动作强度和正式裁决隔离断言。
...
1. 新增 `completed.md` B51 `virtual-judge-npc-beta-readiness-productization-stage-closure`，沉淀本阶段主体成果。
2. 更新 `todo.md` C47，仅保留真实 Beta/staging canary + dashboard 补证，以及 `pause_suggestion` 实现决策两项后置债。
3. 归档活动计划到 `docs/dev_plan/archive/虚拟裁判/20260504T122329Z-virtual-judge-npc-beta-readiness-stage-closure.md`。
4. 将 [当前开发计划.md](/Users/panyihang/Documents/EchoIsle/docs/dev_plan/当前开发计划.md) 与原活动计划重置为空档指针。
...
- 2026-05-04：完成 P4-J `virtual-judge-npc-beta-stage-closure`；本阶段成果已写入 `completed.md` B51，剩余真实环境 / 暂停建议实现边界已写入 `todo.md` C47，活动计划已归档并重置。
