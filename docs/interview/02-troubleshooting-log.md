...
## sse-ticket-reconnect-resilience-v1 | 2026-02-22 23:56:49 -0800
- 改动概述: 第四模块完成：前端 SSE 链路新增自动重连与 ticket 自动轮转。store 增加 access ticket 刷新缓存、指数退避重连调度；utils.initSSE 增加 onOpen/onError 回调；notify_ticket 中间件补充缺 token 返回 401 的单测，避免语义回归。
- 问题 -> 修复:
- 现象/根因: SSE ticket 过期后旧实现会直接 close 导致实时链路静默中断
  修复: 增加 scheduleSSEReconnect（指数退避 + 抖动）并在重连前 refreshAccessTickets
- 现象/根因: 缺少语义测试保障 query 缺 token 分支
  修复: 在 notify middlewares 新增 401 单测
- 验证结果:
- cd chat && cargo test -p notify-server verify_notify_ticket_middleware_should_only_accept_notify_ticket_query -- --nocapture -> pass;cd chat && cargo test -p notify-server verify_notify_ticket_middleware_should_return_401_when_missing_query_token -- --nocapture -> pass;bash skills/post-module-test-guard/scripts/test_change_guard.sh --changes 'chatapp/src/store/index.js;chatapp/src/utils.js;chat/notify_server/src/middlewares.rs' -> pass;bash skills/post-module-test-guard/scripts/run_test_gate.sh --mode quick -> blocked by utoipa-swagger-ui download (github network in runtime)
