...
- 提交: `bcaf0a3`
- 改动概述:
- 第四模块完成：前端 SSE 链路新增自动重连与 ticket 自动轮转。store 增加 access ticket 刷新缓存、指数退避重连调度；utils.initSSE 增加 onOpen/onError 回调；notify_ticket 中间件补充缺 token 返回 401 的单测，避免语义回归。
- 改动文件:
- chatapp/src/store/index.js
- chatapp/src/utils.js
- chat/notify_server/src/middlewares.rs
