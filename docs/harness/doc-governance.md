...
5. 供应链预发故障注入演练报告
6. 供应链 SBOM 与许可证证明报告
7. AI Judge runtime ops pack 收口摘要（`docs/loadtest/evidence/ai_judge_runtime_ops_pack.{env,md}`）
8. AI Judge stage closure evidence 收口摘要（`docs/loadtest/evidence/ai_judge_stage_closure_evidence.{env,md}`）
...
5. 当前开发 harness 规则或脚本时，也必须同步主计划文档
