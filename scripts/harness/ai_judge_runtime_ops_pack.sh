...
P41_CONTROL_PLANE_STATUS="missing"
P41_RUNTIME_READINESS_STATUS="missing"
P41_CHAT_PROXY_STATUS="missing"
P41_FRONTEND_CONTRACT_STATUS="missing"
P41_CALIBRATION_DECISION_LOG_STATUS="missing"
P41_PANEL_SHADOW_CANDIDATE_STATUS="missing"
P41_RUNTIME_OPS_PACK_STATUS="missing"
...
detect_release_readiness_artifact() {
  ...
  P41_CONTROL_PLANE_STATUS="$(trim "$(read_json_string "$RELEASE_READINESS_ARTIFACT_SUMMARY_PATH" "p41ControlPlaneStatus")")"
  P41_RUNTIME_READINESS_STATUS="$(trim "$(read_json_string "$RELEASE_READINESS_ARTIFACT_SUMMARY_PATH" "p41RuntimeReadinessStatus")")"
  P41_CHAT_PROXY_STATUS="$(trim "$(read_json_string "$RELEASE_READINESS_ARTIFACT_SUMMARY_PATH" "p41ChatProxyStatus")")"
  P41_FRONTEND_CONTRACT_STATUS="$(trim "$(read_json_string "$RELEASE_READINESS_ARTIFACT_SUMMARY_PATH" "p41FrontendContractStatus")")"
  P41_CALIBRATION_DECISION_LOG_STATUS="$(trim "$(read_json_string "$RELEASE_READINESS_ARTIFACT_SUMMARY_PATH" "p41CalibrationDecisionLogStatus")")"
  P41_PANEL_SHADOW_CANDIDATE_STATUS="$(trim "$(read_json_string "$RELEASE_READINESS_ARTIFACT_SUMMARY_PATH" "p41PanelShadowCandidateStatus")")"
  P41_RUNTIME_OPS_PACK_STATUS="$(trim "$(read_json_string "$RELEASE_READINESS_ARTIFACT_SUMMARY_PATH" "p41RuntimeOpsPackStatus")")"
  if [[ -z "$P41_CONTROL_PLANE_STATUS" ]]; then P41_CONTROL_PLANE_STATUS="missing"; fi
  if [[ -z "$P41_RUNTIME_READINESS_STATUS" ]]; then P41_RUNTIME_READINESS_STATUS="missing"; fi
  if [[ -z "$P41_CHAT_PROXY_STATUS" ]]; then P41_CHAT_PROXY_STATUS="missing"; fi
  if [[ -z "$P41_FRONTEND_CONTRACT_STATUS" ]]; then P41_FRONTEND_CONTRACT_STATUS="missing"; fi
  if [[ -z "$P41_CALIBRATION_DECISION_LOG_STATUS" ]]; then P41_CALIBRATION_DECISION_LOG_STATUS="missing"; fi
  if [[ -z "$P41_PANEL_SHADOW_CANDIDATE_STATUS" ]]; then P41_PANEL_SHADOW_CANDIDATE_STATUS="missing"; fi
  if [[ -z "$P41_RUNTIME_OPS_PACK_STATUS" ]]; then P41_RUNTIME_OPS_PACK_STATUS="missing"; fi
...
run_stage() {
  ...
  if [[ "$stage" == "fairness" ]]; then
    if [[ "$FAIRNESS_INGEST_ENABLED" == "true" ]]; then
      cmd+=(--ingest-enabled)
    fi
    if [[ -n "$FAIRNESS_INGEST_BASE_URL" ]]; then
      cmd+=(--ingest-base-url "$FAIRNESS_INGEST_BASE_URL")
    fi
    if [[ -n "$FAIRNESS_INGEST_PATH" ]]; then
      cmd+=(--ingest-path "$FAIRNESS_INGEST_PATH")
    fi
    if [[ -n "$FAIRNESS_INGEST_INTERNAL_KEY" ]]; then
      cmd+=(--ingest-internal-key "$FAIRNESS_INGEST_INTERNAL_KEY")
    fi
    if [[ -n "$FAIRNESS_INGEST_TIMEOUT_SECS" ]]; then
      cmd+=(--ingest-timeout-secs "$FAIRNESS_INGEST_TIMEOUT_SECS")
    fi
    if [[ "$FAIRNESS_INGEST_REQUIRE_SUCCESS" == "true" ]]; then
      cmd+=(--ingest-require-success)
    fi
    if [[ -n "$FAIRNESS_INGEST_REPORTED_BY" ]]; then
      cmd+=(--ingest-reported-by "$FAIRNESS_INGEST_REPORTED_BY")
    fi
  fi
...
P41_CONTROL_PLANE_STATUS=$P41_CONTROL_PLANE_STATUS
P41_RUNTIME_READINESS_STATUS=$P41_RUNTIME_READINESS_STATUS
P41_CHAT_PROXY_STATUS=$P41_CHAT_PROXY_STATUS
P41_FRONTEND_CONTRACT_STATUS=$P41_FRONTEND_CONTRACT_STATUS
P41_CALIBRATION_DECISION_LOG_STATUS=$P41_CALIBRATION_DECISION_LOG_STATUS
P41_PANEL_SHADOW_CANDIDATE_STATUS=$P41_PANEL_SHADOW_CANDIDATE_STATUS
P41_RUNTIME_OPS_PACK_STATUS=$P41_RUNTIME_OPS_PACK_STATUS
...
| 子模块 | 状态 | 阈值决策 | 退出码 |
| --- | --- | --- | --- |
| fairness benchmark freeze | \`$FAIRNESS_STATUS\` | \`$FAIRNESS_THRESHOLD_DECISION\`（ingest=\`$FAIRNESS_INGEST_STATUS\`） | \`$FAIRNESS_EXIT_CODE\` |
...
1. control_plane_status：\`$P41_CONTROL_PLANE_STATUS\`
2. runtime_readiness_status：\`$P41_RUNTIME_READINESS_STATUS\`
3. chat_proxy_status：\`$P41_CHAT_PROXY_STATUS\`
4. frontend_contract_status：\`$P41_FRONTEND_CONTRACT_STATUS\`
5. calibration_decision_log_status：\`$P41_CALIBRATION_DECISION_LOG_STATUS\`
6. panel_shadow_candidate_status：\`$P41_PANEL_SHADOW_CANDIDATE_STATUS\`
7. runtime_ops_pack_status：\`$P41_RUNTIME_OPS_PACK_STATUS\`
EOF_MD
}
  ...
  cat >"$EMIT_JSON" <<EOF_JSON
{
  ...
  "stages": [
    {
      ...
      "ingest_status": "$(json_escape "$FAIRNESS_INGEST_STATUS")",
  ...
  "p41_control_plane": {
    "status": "$(json_escape "$P41_CONTROL_PLANE_STATUS")",
    "runtime_readiness_status": "$(json_escape "$P41_RUNTIME_READINESS_STATUS")",
    "chat_proxy_status": "$(json_escape "$P41_CHAT_PROXY_STATUS")",
    "frontend_contract_status": "$(json_escape "$P41_FRONTEND_CONTRACT_STATUS")",
    "calibration_decision_log_status": "$(json_escape "$P41_CALIBRATION_DECISION_LOG_STATUS")",
    "panel_shadow_candidate_status": "$(json_escape "$P41_PANEL_SHADOW_CANDIDATE_STATUS")",
    "runtime_ops_pack_status": "$(json_escape "$P41_RUNTIME_OPS_PACK_STATUS")"
  }
}
EOF_JSON
}
...
1. fairness_benchmark_freeze: \`$FAIRNESS_STATUS\` (threshold=\`$FAIRNESS_THRESHOLD_DECISION\`, ingest=\`$FAIRNESS_INGEST_STATUS\`, exit=\`$FAIRNESS_EXIT_CODE\`)
...
1. status: \`$P41_CONTROL_PLANE_STATUS\`
2. runtime_readiness_status: \`$P41_RUNTIME_READINESS_STATUS\`
3. chat_proxy_status: \`$P41_CHAT_PROXY_STATUS\`
4. frontend_contract_status: \`$P41_FRONTEND_CONTRACT_STATUS\`
5. calibration_decision_log_status: \`$P41_CALIBRATION_DECISION_LOG_STATUS\`
...
main() {
  ...
  FAIRNESS_INGEST_STATUS="$(trim "$(read_env_value "$fairness_env" "FAIRNESS_INGEST_STATUS")")"
  ...
  echo "fairness_ingest_status: $FAIRNESS_INGEST_STATUS"
  ...
  echo "p41_control_plane_status: $P41_CONTROL_PLANE_STATUS"
  echo "p41_runtime_readiness_status: $P41_RUNTIME_READINESS_STATUS"
  echo "p41_chat_proxy_status: $P41_CHAT_PROXY_STATUS"
  echo "p41_frontend_contract_status: $P41_FRONTEND_CONTRACT_STATUS"
  echo "p41_calibration_decision_log_status: $P41_CALIBRATION_DECISION_LOG_STATUS"
  echo "p41_panel_shadow_candidate_status: $P41_PANEL_SHADOW_CANDIDATE_STATUS"
  echo "p41_runtime_ops_pack_status: $P41_RUNTIME_OPS_PACK_STATUS"
