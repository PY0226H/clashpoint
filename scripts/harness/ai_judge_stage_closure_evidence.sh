...
detect_runtime_ops_link() {
  if [[ -f "$RUNTIME_OPS_ENV" && -f "$RUNTIME_OPS_DOC" ]]; then
    ...
    P41_CONTROL_PLANE_STATUS="$(trim "$(read_env_value "$RUNTIME_OPS_ENV" "P41_CONTROL_PLANE_STATUS")")"
    P41_RUNTIME_READINESS_STATUS="$(trim "$(read_env_value "$RUNTIME_OPS_ENV" "P41_RUNTIME_READINESS_STATUS")")"
    P41_CHAT_PROXY_STATUS="$(trim "$(read_env_value "$RUNTIME_OPS_ENV" "P41_CHAT_PROXY_STATUS")")"
    P41_FRONTEND_CONTRACT_STATUS="$(trim "$(read_env_value "$RUNTIME_OPS_ENV" "P41_FRONTEND_CONTRACT_STATUS")")"
    P41_CALIBRATION_DECISION_LOG_STATUS="$(trim "$(read_env_value "$RUNTIME_OPS_ENV" "P41_CALIBRATION_DECISION_LOG_STATUS")")"
    P41_PANEL_SHADOW_CANDIDATE_STATUS="$(trim "$(read_env_value "$RUNTIME_OPS_ENV" "P41_PANEL_SHADOW_CANDIDATE_STATUS")")"
    P41_RUNTIME_OPS_PACK_STATUS="$(trim "$(read_env_value "$RUNTIME_OPS_ENV" "P41_RUNTIME_OPS_PACK_STATUS")")"
